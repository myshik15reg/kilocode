...
1. **Set working directory**: Use the runtime's explicit working-directory parameter (for example `workdir` or `cwd`)
2. **Chain commands safely (shell-dependent)**:
    - Windows PowerShell 5.1: avoid relying on `&&` / `||`; prefer separate commands or explicit `$LASTEXITCODE` checks
    - PowerShell 7+: supports `&&` / `||`, but this pack avoids them for portability; use explicit checks
    - bash/zsh: use `set -euo pipefail` and separate commands (or explicit `if` checks) for clear error handling
3. **Handle paths**: Use relative paths when possible, avoid ~ or $HOME

**Example of proper command preparation:**

```text
# PowerShell 5.1 (Windows default): run commands separately
npm install
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
npm run build

# PowerShell 5.1: explicit working directory
cd .\\frontend
npm run dev

# bash/zsh (Linux/macOS): explicit error handling
set -euo pipefail
npm install
npm run build
cd ./frontend
...
Use the active shell-execution tool with an explicit command and working directory.
...
PowerShell 5.1 does **not** support `&&` / `||`. Prefer explicit, portable patterns.

**PowerShell 5.1 (recommended for Windows):**
...
3. If commands fail, document the issues and decide whether to:
    ...
    - Record the known issue in `execution.md` and defer it explicitly when needed
...
# Step 3: Execute with the shell tool
command: npm install
workdir: .
...
workdir: .
# Step 3: Create a new branch if the task explicitly requires it
