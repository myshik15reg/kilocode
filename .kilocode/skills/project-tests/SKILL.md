...
2. Inspect `package.json` to identify available test scripts and dependencies.
...
1. Check if dependencies are installed:
    ...
    ```
2. If dependencies are missing and the task scope allows environment setup, install them:
    ```bash
    npm install
    # or
    yarn install
...
docker volume create --driver local --opt type=tmpfs --opt device=tmpfs --opt o=size=1g test-db-volume

# Запуск контейнера с БД в RAM
docker run -d --name test-db -v test-db-volume:/var/lib/postgresql/data -e POSTGRES_PASSWORD=test postgres:15

# После тестов (без `&&`, совместимо с PowerShell 5.1)
docker rm -f test-db
