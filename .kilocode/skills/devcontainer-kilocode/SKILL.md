...
description: Настройка devcontainer с bind mounts для Alfa Code (skills/modes) на Windows.
---

# Навык: Devcontainer Alfa Code

## Назначение

Навык описывает воспроизводимую настройку devcontainer для работы с Alfa Code, включая проброс локальной папки `~/.kilocode` и файла `custom_modes.yaml` через bind mounts. Это снижает риск несовпадения локальных настроек и повышает повторяемость окружения.

## Триггеры

Используй этот навык, когда:

- нужно подготовить devcontainer для работы с Alfa Code
- требуется доступ контейнера к локальным настройкам `~/.kilocode`
- нужно пробросить `custom_modes.yaml` в контейнер
- возникают расхождения между локальной и контейнерной конфигурацией Alfa Code
...
- `../../workflows/create-new-skill.md` — убедиться в соответствии структуре skills
- `../../rules/environment-windows.md` — правила Windows путей и quoting
...
- `KILOCODE_GLOBAL_PATH` — путь к Alfa Code внутри контейнера (например, `/root/.kilocode`)
...
3. При изменении структуры Alfa Code обнови `.kilocode/memory-bank/architecture.md`.
...
```json
{
	...
	"customizations": {
		"vscode": {
			"extensions": ["kilocode.kilo-code", "ms-azuretools.vscode-docker"],
			"settings": {
	...
	"postStartCommand": "echo '🚀 Alfa Code devcontainer is ready! 🐳 Docker CLI available for managing external containers'",
