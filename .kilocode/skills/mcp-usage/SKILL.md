...
Перед тем как подключать MCP-related capability в workflow/docs/handoff, классифицируй используемую surface через [`../../patterns/orchestration/integration-types.md`](../../patterns/orchestration/integration-types.md:1):

1. `resource` - read-only context or corpus.
2. `prompt` - reusable template or bridge layer.
3. `tool` - executable MCP action.

Этот guide сейчас описывает прежде всего `tool`-surfaces. Если сервер позже используется ещё и как `resource` или `prompt`, эти surfaces MUST быть описаны отдельно, а не смешаны в один blob.
...
**Режимы:** `orchestrator`, `planning-research-*`, `solution-architect`, `debug`
