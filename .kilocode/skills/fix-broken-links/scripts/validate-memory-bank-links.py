...
1. Scans .kilocode/memory-bank/ directory
...
def resolve_link(source_file: Path, link_target: str, base_dir: Path) -> Path:
    ...
    - Absolute paths from repo root: /.kilocode/memory-bank/file.md
    """
    # Remove anchor fragments and editor line/column suffixes like :12 or :12:3
    if '#' in link_target:
        link_target = link_target.split('#')[0]
    link_target = re.sub(r':\d+(?::\d+)?$', '', link_target)
