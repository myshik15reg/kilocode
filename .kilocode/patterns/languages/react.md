...
export const UserCard: FC<UserCardProps> = memo(({ user, isEditable = false, onUpdate, children }) => {
...
// Selectors
export const useActiveUsers = () => useUserStore((state) => state.users.filter((u) => u.isActive))

export const useUserById = (id: number) => useUserStore((state) => state.users.find((u) => u.id === id))
```
...
function UserList() {
	...
	return (
		<div>
			...
			<button onClick={() => createUser.mutate({ name: "New User" })} disabled={createUser.isPending}>
				Add User
			</button>
		</div>
	)
