...
const query = new QueryBuilder<User>().select("id", "name").where({ name: "John" }).orderBy("id").build()
```
...
type ReturnTypeOfPromise<T extends (...args: any[]) => Promise<any>> = T extends (...args: any[]) => Promise<infer R>
	? R
	: never
```
