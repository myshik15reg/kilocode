...
<template>
	<div class="user-card">
		<h2>{{ displayName }}</h2>
		<input v-if="isEditable" v-model="inputValue" :maxlength="maxLength" @keyup.enter="handleUpdate" />
...
export function useFetch<T>(url: string | (() => string), options: UseFetchOptions<T> = {}): UseFetchReturn<T> {
...
export const useUserStore = defineStore("user", () => {
	...
	const userById = computed(() => (id: number) => users.value.find((u) => u.id === id))
...
describe("UserCard", () => {
	...
	it("emits update event when button clicked", async () => {
		...
		expect(wrapper.emitted("update")?.[0]).toEqual([expect.objectContaining({ id: 1 })])
	})
