...
// С зависимостями
const route = useRoute()
const { data } = await useAsyncData(`post-${route.params.id}`, () => $fetch(`/api/posts/${route.params.id}`), {
...
<template>
	<div>
		<p v-if="userStore.isAuthenticated">Welcome, {{ userStore.fullName }}!</p>
...
```vue
<template>
	<NuxtImg src="/images/hero.jpg" alt="Hero image" width="800" height="600" loading="lazy" format="webp" />
</template>
```
