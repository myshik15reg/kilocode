...
Этот документ содержит **смешанный** материал:

- ✅ **Firm-SoT**: только разделы, помеченные как _Firm-SoT_ (ниже) и имеющие ссылки на источники.
- ⚠️ **Non-firm**: все остальные разделы — справочные/примерные и **не должны** восприниматься как фирменный стандарт.
...
## Firm-SoT: ключевые правила

1. **Единое хранилище правил** — база 1С: «Конвертация данных», ред. 3.0. Изменения нужно переносить в КД 3.0.

    - Источник: [`KD3`](../../sources/1c-alfa/kd3.md#key-rules)

2. **Существующие правила не переименовывать** (в отдельных интеграциях поиск выполняется по имени правила).

    - Источник: [`KD3`](../../sources/1c-alfa/kd3.md#key-rules)

3. Изменения программного кода правил должны вноситься с учётом общего регламента разработки 1С (оформление модулей/запросов/комментариев и т.п.).
    - Источник: [`KD3`](../../sources/1c-alfa/kd3.md#key-rules)
...
Источник: [`KD3`](../../sources/1c-alfa/kd3.md#pipelines)

### Обработка пакета (загрузка)

Порядок этапов:

1. `КодПередОбработкойПакета`
2. `КодПередПолучениемОбъекта`
3. поиск объекта (GUID / дополнительный поиск)
4. определение ссылки нового объекта (если объект не найден)
...
> ⚠️ Non-firm: справочный раздел. Для firm-правил ориентируйся на разделы _Firm-SoT_ выше.
...
> ⚠️ Non-firm: последовательности событий ниже могут не совпадать с фирменными этапами выполнения.
> Firm-SoT пайплайны см. выше: [`KD3`](../../sources/1c-alfa/kd3.md#pipelines)
...
> ⚠️ Non-firm: примеры ниже иллюстративны и не являются фирменным стандартом.
