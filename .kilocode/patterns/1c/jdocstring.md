...
## Технические маркеры изменений (BSL)

`///` (jdocstring) используется **только** для документационных комментариев (API, параметры, примеры).

Для трассировки правок в BSL используй **маркер изменений Альфа-Лизинг** (обычные комментарии `//`, отдельные от docstring).

SoT и точные шаблоны см. в: [`kilocode-change-marker.md`](kilocode-change-marker.md).

> Примечание (legacy): в истории репозитория могут встречаться маркеры вида `// kilocode_change: [...] ...`.
> Это допустимо для истории, но **для новых правок в BSL** применяется только формат Альфа-Лизинг.
