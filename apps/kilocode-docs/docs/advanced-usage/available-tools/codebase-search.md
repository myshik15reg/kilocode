...
- When AlfaCode assistant needs to find code related to specific functionality across your project
...
- When implementing a new feature, AlfaCode assistant searches for "authentication middleware" to understand existing patterns before writing new code.
- When debugging an issue, AlfaCode assistant searches for "error handling in API calls" to find related error patterns across the codebase.
- When refactoring code, AlfaCode assistant searches for "database transaction patterns" to ensure consistency across all database operations.
- When onboarding to a new codebase, AlfaCode assistant searches for "configuration loading" to understand how the application bootstraps.
