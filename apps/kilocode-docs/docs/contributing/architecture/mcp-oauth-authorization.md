...
Many MCP servers require authentication to access protected resources. Currently, AlfaCode assistant only supports static credential configuration (API keys, tokens) which must be manually entered and stored. This creates friction for users and security concerns for enterprises.

The MCP specification defines an OAuth 2.1-based authorization flow that enables secure, user-friendly authentication without requiring users to manually manage credentials. This document specifies how AlfaCode assistant will implement the MCP Authorization specification to support OAuth-enabled MCP servers.
...
- **MCP Client** (AlfaCode assistant) - Acts as OAuth 2.1 Client, obtains tokens on behalf of users
...
For Client ID Metadata Documents, AlfaCode assistant needs to host a metadata document. We will use static hosting on kilocode.ai:
...
```json
{
	...
	"redirect_uris": ["http://127.0.0.1:0/callback", "vscode://kilocode.alfa-code-assistant/oauth/callback"],
...
2. **VS Code URI Handler** (Fallback)
    - Register `vscode://kilocode.alfa-code-assistant/oauth/callback` URI handler
...
- [ ] Host AlfaCode assistant client metadata at kilocode.ai
