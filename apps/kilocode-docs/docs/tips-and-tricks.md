...
A collection of quick tips to help you get the most out of AlfaCode assistant.

- Right-click on the AlfaCode assistant icon in the Activity Bar and select **Move To → Secondary Side Bar** to move AlfaCode assistant to the [Secondary Sidebar](https://code.visualstudio.com/api/ux-guidelines/sidebars#secondary-sidebar), so you can see the Explorer, Search, Source Control, etc. alongside AlfaCode assistant.

    ![Move to Secondary Side Bar](./move-to-secondary.png)

- Once you have AlfaCode assistant in a separate sidebar from the file explorer, you can drag files from the explorer into the chat window (and even multiple at once). Just make sure to hold down the shift key after you start dragging the files.
- **Autocomplete Tips:**
...
- If you want to really accelerate, check out multiple copies of your repository and run AlfaCode assistant on all of them in parallel (using git to resolve any conflicts, same as with human devs).
