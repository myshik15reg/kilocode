...
Footgun Prompting, AKA Overriding System Prompt, allows advanced users to completely replace the default system prompt for a specific AlfaCode assistant mode. This provides granular control over the AI's behavior but bypasses built-in safeguards.
...
3.  **Activation:** AlfaCode assistant automatically detects this file. When present, it replaces most of the standard system prompt sections.
...
You can find the option and instructions within the AlfaCode assistant UI:

1.  Click the MODE selector in the bottom-left of the AlfaCode assistant text-input box.
...
- **Intended Audience:** Best suited for users deeply familiar with AlfaCode assistant's prompting system and the implications of modifying core instructions.
...
- **No File, No Override:** If the `.kilocode/system-prompt-{mode-slug}` file doesn't exist, AlfaCode assistant uses the standard system prompt generation process for that mode.
- **Directory Creation:** AlfaCode assistant ensures the `.kilocode` directory exists before attempting to read or create the override file.

Use this feature cautiously. While powerful for customization, incorrect implementation can significantly degrade AlfaCode assistant's performance and reliability for the affected mode.
