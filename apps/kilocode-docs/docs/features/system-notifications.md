...
AlfaCode assistant uses system notifications to inform you about:
...
AlfaCode assistant's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. AlfaCode assistant uses macOS's built-in command to display notifications.
...
**How it works:** AlfaCode assistant first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
