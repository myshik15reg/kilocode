...
The `search_files` tool performs regex searches across multiple files in your project. It helps AlfaCode assistant locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When AlfaCode assistant needs to find where specific functions or variables are used
- When AlfaCode assistant helps with refactoring and needs to understand usage patterns
- When AlfaCode assistant needs to locate all instances of a particular code pattern
- When AlfaCode assistant searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, AlfaCode assistant first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, AlfaCode assistant searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, AlfaCode assistant locates all TODO comments across the project.
- When analyzing dependencies, AlfaCode assistant finds all imports of a particular module.
