...
足枪式提示，又称覆盖系统提示，允许高级用户完全替换特定 AlfaCode assistant 模式的默认系统提示。这提供了对 AI 行为的精细控制，但绕过了内置的安全措施。
...
3.  **激活：** AlfaCode assistant 会自动检测此文件。如果存在，它将替换大多数标准系统提示部分。
...
您可以在 AlfaCode assistant UI 中找到选项和说明：

1.  单击 AlfaCode assistant 文本输入框左下角的 MODE 选择器。
2.  单击模式选择列表底部的“编辑...”
...
- **目标受众：** 最适合深入了解 AlfaCode assistant 提示系统以及修改核心指令所带来影响的用户。
- **对功能的影响：** 自定义提示会覆盖标准指令，包括工具使用和响应一致性的指令。如果管理不当，这可能导致意外行为或错误。
- **模式特定：** 每个覆盖文件仅适用于其文件名中指定的模式（`{mode-slug}`）。
- **无文件，无覆盖：** 如果 `.kilocode/system-prompt-{mode-slug}` 文件不存在，AlfaCode assistant 将使用该模式的标准系统提示生成过程。
- **目录创建：** AlfaCode assistant 确保 `.kilocode` 目录存在，然后才尝试读取或创建覆盖文件。

请谨慎使用此功能。虽然它在自定义方面功能强大，但错误的实现可能会显著降低 AlfaCode assistant 在受影响模式下的性能和可靠性。
