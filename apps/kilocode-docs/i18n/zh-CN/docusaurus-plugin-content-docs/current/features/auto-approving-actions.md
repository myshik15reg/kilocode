...
> ⚠️ **安全警告：** 自动批准设置会绕过确认提示，使 AlfaCode assistant 直接访问您的系统。这可能导致**数据丢失、文件损坏，甚至更糟**。命令行访问尤其危险，因为它可能执行有害操作，从而损坏您的系统或危及安全。仅对您完全信任的操作启用自动批准。
...
1.  单击聊天输入上方的自动批准工具栏
2.  选择 AlfaCode assistant 可以执行哪些操作而无需请求权限
...
| 权限                 | 作用                                | 风险级别 |
| -------------------- | ----------------------------------- | -------- |
| **读取文件和目录**   | 允许 AlfaCode assistant 无需询问即可访问文件 | 中       |
| **编辑文件**         | 允许 AlfaCode assistant 无需询问即可修改文件 | **高**   |
| **执行已批准的命令** | 自动运行白名单中的终端命令          | **高**   |
| **使用浏览器**       | 允许无头浏览器交互                  | 中       |
| **使用 MCP 服务器**  | 允许 AlfaCode assistant 使用配置的 MCP 服务  | 中-高    |
| **切换模式**         | 自动在 AlfaCode assistant 模式之间切换       | 低       |
...
> **允许 AlfaCode assistant 自动执行操作而无需批准。仅在您完全信任 AI 并了解相关安全风险的情况下才启用这些设置。**
...
**描述：** “启用后，AlfaCode assistant 将自动查看目录内容并读取文件，无需您单击批准按钮。”

**风险级别：** 中

虽然此设置只允许读取文件（不修改它们），但它可能会暴露敏感数据。对于大多数用户来说，仍然建议将其作为起点，但请注意 AlfaCode assistant 可以访问哪些文件。
:::
...
此设置允许 AlfaCode assistant 无需确认即可修改您的文件。延迟计时器至关重要：
...
_AlfaCode assistant 在写入延迟期间检查的 VSCode 问题窗格_

当您启用文件写入的自动批准时，延迟计时器与 VSCode 的问题窗格一起工作：

1.  AlfaCode assistant 对您的文件进行更改
2.  VSCode 的诊断工具分析更改
3.  问题窗格更新任何错误或警告
4.  AlfaCode assistant 在继续之前注意到这些问题
...
允许 AlfaCode assistant 无需确认即可控制无头浏览器。这可以包括：
...
此设置会在 API 调用失败时自动重试。延迟控制 AlfaCode assistant 在重试之前等待多长时间：
...
允许 AlfaCode assistant 在不同模式（代码、架构师等）之间切换，无需请求权限。这主要影响 AI 的行为，而不是系统访问。
:::
...
启用 AlfaCode assistant 自动创建和完成子任务。这与工作流组织有关，而不是系统访问。
:::
...
**描述：** 在可配置的超时后，自动为后续问题选择第一个 AI 建议的答案。这通过让 AlfaCode assistant 无需手动干预即可继续操作来加快您的工作流程。
...
- 您希望 AlfaCode assistant 继续工作的隔夜运行
- 默认建议通常正确的重复任务
- 交互不重要的测试工作流
  :::
...
此设置允许 AlfaCode assistant 在工作会话期间自动更新任务进度和待办事项列表。这包括：
...
当与子任务权限结合使用时，这尤其有用，因为它允许 AlfaCode assistant 在没有持续批准请求的情况下保持项目进度的完整视图。
:::
