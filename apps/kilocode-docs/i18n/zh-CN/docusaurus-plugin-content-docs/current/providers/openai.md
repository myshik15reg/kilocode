...
# 在 AlfaCode assistant 中使用 OpenAI

AlfaCode assistant 支持通过官方 OpenAI API 直接访问模型。
...
3. **创建密钥:** 点击 "Create new secret key"。为密钥起一个描述性名称（如 "AlfaCode assistant"）
4. **复制密钥:** **重要:** 立即复制 API 密钥。之后将无法再次查看。请安全存储

## 支持的模型

AlfaCode assistant 支持多种 OpenAI 模型，包括：
