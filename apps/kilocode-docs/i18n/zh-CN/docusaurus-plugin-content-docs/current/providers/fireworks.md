...
# 在 AlfaCode assistant 中使用 Fireworks AI
...
3. **创建密钥：** 点击 "Create API key" 并为您的密钥输入一个描述性名称（例如 "AlfaCode assistant"）。
...
AlfaCode assistant 支持以下 Fireworks AI 模型：
...
1. **打开 AlfaCode assistant 设置：** 在 AlfaCode assistant 面板中点击齿轮图标 (<Codicon name="gear" />)。
