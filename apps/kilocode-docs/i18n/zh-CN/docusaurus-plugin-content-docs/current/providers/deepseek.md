...
# 在AlfaCode assistant中使用DeepSeek

AlfaCode assistant支持通过DeepSeek API访问模型，包括`deepseek-chat`和`deepseek-reasoner`。
...
3. **创建密钥:** 点击"Create new API key"。为密钥起一个描述性名称(例如"AlfaCode assistant")。
4. **复制密钥:** **重要:** 立即复制API密钥，之后将无法再次查看。请安全存储。

## 支持的模型

AlfaCode assistant支持以下DeepSeek模型：
