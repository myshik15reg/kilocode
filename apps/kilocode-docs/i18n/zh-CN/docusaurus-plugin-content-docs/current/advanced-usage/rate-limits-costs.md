...
了解和管理 API 使用情况对于在 AlfaCode assistant 中获得流畅且经济高效的体验至关重要。本节将解释如何跟踪你的 token 使用情况、成本以及如何配置速率限制。

## Token 使用

AlfaCode assistant 使用 token 与 AI 模型进行交互。Token 本质上是词语的组成部分。请求和响应中使用的 token 数量会影响处理时间和成本。
...
AlfaCode assistant 会根据配置的模型定价自动计算每次 API 请求的预估成本。该成本显示在聊天记录中，紧邻 token 使用情况。
...
为了防止意外过度使用 API 并帮助你管理成本，AlfaCode assistant 允许你设置速率限制。速率限制定义了 API 请求之间的最短时间（以秒为单位）。
...
如果你将速率限制设置为 10 秒，AlfaCode assistant 会在一个 API 请求完成后至少等待 10 秒再发送下一个请求。
...
- **使用自定义指令：**提供自定义指令以指导 AlfaCode assistant 的行为，减少每次提示中冗长的解释。
...
通过了解和管理你的 API 使用情况，你可以高效且经济地使用 AlfaCode assistant。
