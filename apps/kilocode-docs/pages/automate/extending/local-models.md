...
AlfaCode assistant supports running language models locally on your own machine using [Ollama](https://ollama.com/) and [LM Studio](https://lmstudio.ai/). This offers several advantages:

- **Privacy:** Your code and data never leave your computer.
- **Offline Access:** You can use AlfaCode assistant even without an internet connection.
...
AlfaCode assistant currently supports two main local model providers:

1.  **Ollama:** A popular tool for running large language models locally. It supports a wide range of models.
...
- **"No connection could be made because the target machine actively refused it":** This usually means that the Ollama or LM Studio server isn't running, or is running on a different port/address than AlfaCode assistant is configured to use. Double-check the Base URL setting.
