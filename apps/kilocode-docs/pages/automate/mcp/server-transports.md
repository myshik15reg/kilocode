...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between AlfaCode assistant and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (AlfaCode assistant) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as AlfaCode assistant
...
- Single-client scenarios (one AlfaCode assistant instance per server)
...
1. The client (AlfaCode assistant) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from AlfaCode assistant
...
STDIO servers run locally on the same machine as AlfaCode assistant, which has several important implications:
...
- **Execution**: Starts and stops with AlfaCode assistant (child process lifecycle)
...
- Start when needed by AlfaCode assistant
- Not require network configuration
- Need to be installed alongside AlfaCode assistant or via a package manager
