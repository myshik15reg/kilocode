...
# Using LM Studio With AlfaCode assistant

AlfaCode assistant supports running models locally using LM Studio. LM Studio provides a user-friendly interface for downloading, configuring, and running local language models. It also includes a built-in local inference server that emulates the OpenAI API, making it easy to integrate with AlfaCode assistant.
...
2.  **Download a Model:** Use the LM Studio interface to search for and download a model. Some recommended models include:
    ...
    - Any other model that is supported by AlfaCode assistant, or for which you can set the context window.
...
5.  **(Optional) Timeout:** By default, API requests time out after 10 minutes. Local models can be slow, if you hit this timeout you can consider increasing it here: VS Code Extensions panel > AlfaCode assistant gear menu > Settings > API Request Timeout.
...
- **Local Server:** The LM Studio local server must be running for AlfaCode assistant to connect to it.
