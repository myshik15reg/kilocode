...
describe("TaskItemFooter", () => {
	...
	it("posts only pause action for running history items", () => {
		render(<TaskItemFooter item={{ ...mockItem, lifecycleState: "running" }} variant="full" />)
		...
		expect(postMessageMock).toHaveBeenCalledWith({ type: "pauseTask", text: "1" })
	})
	...
	it("shows only resume control for stopped history items", () => {
		render(
			<TaskItemFooter
				item={{ ...mockItem, status: "active", lastStopReason: "user_cancelled" }}
				variant="full"
			/>,
		)
