...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>((props, ref) => {
	...
	const pendingSavedStateRef = useRef<Partial<ExtensionStateContextType> | null>(null)

	const OUT_OF_BAND_SAVE_KEYS = [
		"ttsEnabled",
		"ttsSpeed",
	...
	useEffect(() => {
		...
		if (isLoadingProfileForEditing.current) {
			return
		}
		...
		if (!isChangeDetected || !pendingSavedStateRef.current) {
	...
	const handleSubmit = () => {
		if (isSettingValid) {
			const updatedSettings = {
				...
				autoRestartProblematicProcesses: autoRestartProblematicProcesses ?? false,

				problematicProcessRestartLimit: Math.min(Math.max(1, problematicProcessRestartLimit ?? 1), 10), // kilocode_change

				parallelAgentsEnabled: parallelAgentsEnabled ?? false,
			...
			pendingSavedStateRef.current = getRoundTripPendingSettings(
				updatedSettings as Partial<ExtensionStateContextType>,
			)
					...
					<SearchIndexProvider value={searchContextValue}>
						...
						{activeTab === "contextManagement" && (
							<ContextManagementSettings
								...
								contextRoutingEnabled={contextRoutingEnabled ?? true} // kilocode_change
								contextRoutingFastThresholdPercent={contextRoutingFastThresholdPercent ?? 35} // kilocode_change
								contextRoutingDeepThresholdPercent={contextRoutingDeepThresholdPercent ?? 65} // kilocode_change
								condensingApiConfigId={condensingApiConfigId}
								listApiConfigMeta={listApiConfigMeta ?? []}
						...
						{activeTab === "alfaCode" && (
							<AlfaCodeSettings
								...
								autoRestartProblematicProcesses={cachedState.autoRestartProblematicProcesses}
								...
								terminalCommandApiConfigId={cachedState.terminalCommandApiConfigId}
								setCachedStateField={setCachedStateField}
							/>
						)}
