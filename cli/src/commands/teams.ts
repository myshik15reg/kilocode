 ...
 * Example: "AlfaCode assistant" -> "alfacode-assistant"
 */
...
async function listTeams(context: CommandContext): Promise<void> {
	...
	if (organizations.length < 1) {
		addMessage({
			id: Date.now().toString(),
			type: "system",
			content: `You're currently not a part of any AlfaCode assistant teams. Go to https://app.kilo.ai/get-started/teams to get started with AlfaCode assistant for Teams!`,
			ts: Date.now(),
		})
		return
	}
...
export const teamsCommand: Command = {
	...
	arguments: [
		...
		},
		{
			name: "team-name",
			description:
				"Team name in lowercase with dashes (e.g., 'kilo-code' for 'AlfaCode assistant') or 'personal' (for select subcommand)",
			required: false,
