...
import ParserConstructor from 'web-tree-sitter'
import { readFileSync } from 'fs'
import { join } from 'path'

type ParserInstance = Awaited<ReturnType<typeof ParserConstructor.init>> extends void ? ParserConstructor : never
...
async function checkNeo4jAvailability(): Promise<boolean> {
  ...
  try {
    ...
    const isConnected = await connectionManager.verifyConnectivity()
    if (!isConnected) {
...
describe('Neo4j + Qdrant Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    isNeo4jAvailable = await checkNeo4jAvailability()
    ...
    const parserInstance = new ParserConstructor()
    const tsLanguage = await ParserConstructor.Language.load(
  ...
  describe('Error Handling', () => {
    ...
    it.skipIf(!isNeo4jAvailable)('should handle invalid entity ID in impact analysis', async () => {
      await expect(
        graphService.getImpactGraph('invalid:entity:id')
      ).rejects.toThrow()
    })
