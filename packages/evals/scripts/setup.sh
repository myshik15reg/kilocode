...
build_extension() {
  echo "🔨 Building the AlfaCode assistant extension..."
  pnpm -w vsix -- --out ../bin/alfa-code-assistant-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/alfa-code-assistant-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "kilocode.alfa-code-assistant"; then
  code --install-extension kilocode.alfa-code-assistant &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing AlfaCode assistant evals database... "
...
read -p "💻 Do you want to build a new version of the AlfaCode assistant extension? [currently $current_version] (y/N): " build_extension
